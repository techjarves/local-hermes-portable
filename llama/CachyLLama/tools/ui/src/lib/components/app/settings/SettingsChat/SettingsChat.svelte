<script lang="ts">
	...
	import { ModelFitDashboard } from '$lib/components/app/model-fit';
	...
	let isFitSection = $derived(activeSlug === SETTINGS_SECTION_SLUGS.FIT);

	let localConfig: SettingsConfigType = $state({ ...config() });

	let mobileHeader: { updateCarousel: () => void } | undefined = $state();
...
<div class="mx-auto flex h-full w-full flex-col md:pl-8" in:fade={{ duration: 150 }}>
	<div class="flex flex-1 flex-col gap-4 md:flex-row">
		...
		<div class={['mx-auto flex-1', isFitSection ? 'w-full max-w-6xl' : 'max-w-3xl']}>
			<div class={['space-y-6 p-4 md:p-6 md:pt-28', isFitSection ? 'pb-6' : '']}>
				<div class="grid">
					...
					{:else if currentSection.title === SETTINGS_SECTION_TITLES.IMPORT_EXPORT}
						<SettingsChatImportExportTab />
					{:else if currentSection.title === SETTINGS_SECTION_TITLES.FIT}
						<div
							class="h-[calc(100vh-15rem)] min-h-[38rem] w-full overflow-hidden rounded-xl border border-border bg-background md:h-[calc(100vh-13rem)]"
						>
							<iframe
								src="http://127.0.0.1:8787/"
								title="llmfit Model Fit"
								class="h-full w-full border-none bg-background"
							></iframe>
						</div>
				...
				<div class={['mt-8 border-t border-border/30 pt-6', isFitSection ? 'hidden' : '']}>
